...
def spam():
	...
	for i in urls:
		count += 1
		try:
			requests.get(i, timeout=2); print(colorama.Fore.YELLOW + "Сообщение номером №{} отправлено на {}".format(count, colorama.Fore.WHITE + spam_id))
		except:
			print(colorama.Fore.RED + "Ошибка сервиса №" + str(count))
...
/_______  /|__|_|  /(____  /|__||____//_______  /|   __/(____  /|__|_|  / 
        \\/       \\/      \\/                   \\/ |__|        \\/       \\/  """ + colorama.Fore.CYAN + "\n                        by @KaliSecurityMax \n               github.com/KaliSecurityMax/EmailFlood \n ----------------------------------------------------------------------\n\n")


spam_id = input(colorama.Fore.MAGENTA + "Введите сюда почту: " + colorama.Fore.WHITE)
...
for i in range(int(input(colorama.Fore.MAGENTA + "Введите количество тем (Пример: 10): " + colorama.Fore.WHITE))):
